...
for t in range(nt):
	spike_pre_ = np.expand_dims(spike_pre[:, t], 0)
	spike_post_ = np.expand_dims(spike_post[:, t], 1)
	x_pre_ = np.expand_dims(x_pre, 0)
	x_post_ = np.expand_dims(x_post, 1)
...
plt.ylabel("$\Delta w$")
plt.xlim(-50, 50)
plt.savefig('results/onlineSTDPs.png')